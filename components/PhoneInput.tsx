...
// Using flagcdn.com — works on all platforms (Windows Chrome doesn't render
// country flag emoji natively).
...
// ─── Country data ─────────────────────────────────────────────────────────────
// Bhutan first, then the 3 INR-rate countries, then all others A–Z.
// (Priority entries are NOT repeated in the main list below.)
...
const REST: PhoneCountry[] = [
  ...
  { name: 'Congo, Republic of',               code: 'CG', dialCode: '+242'   },
...
].filter(c => !PRIORITY_CODES.has(c.code))   // exclude priority countries from main list

const INR_RATE_CODES = new Set(['IN', 'BD', 'MV'])
...
export default function PhoneInput({
  ...
  // Sync external value → local number (strip dial code prefix if present)
  useEffect(() => {
    if (value && value !== selected.dialCode + localNumber) {
      const match = [...PRIORITY, ...REST].find(c => value.startsWith(c.dialCode))
      if (match) {
        setSelected(match)
      ...
      {open && (
        <div className="absolute z-50 mt-1.5 w-72 bg-white border border-stone-200 rounded-xl shadow-lg overflow-hidden">
          ...
          <ul className="max-h-60 overflow-y-auto py-1">
            ...
            {!hasResults && (
              <li className="px-4 py-3 text-sm text-stone-400 text-center">No results</li>
            )}
              ...
              <>
                ...
                {filteredPriority.map(c => (
                  <CountryRow
                    ...
                    isINRRate={INR_RATE_CODES.has(c.code)}
                    isBhutan={c.code === 'BT'}
                    onPick={() => pick(c)}
                  />
                ))}
...
function CountryRow({
  country, selected, isINRRate, isBhutan, onPick,
}: {
  ...
  return (
    <li>
      ...
      >
        ...
        {isBhutan && (
          <span className="shrink-0 text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded-md">
            Home
          </span>
        )}
        {isINRRate && (
          <span className="shrink-0 text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-100 px-1.5 py-0.5 rounded-md">
            INR Rate
          </span>
        )}
        <span className="shrink-0 text-xs text-stone-400 tabular-nums font-mono">{country.dialCode}</span>
      </button>
    </li>
  )
}
